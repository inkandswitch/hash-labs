...
import type { ArcType, PlaceMarkingsById, TransitionCondition, TransitionNodeType } from "./types";

/**
 * Check if a transition is enabled, i.e. that the tokens required by each incoming arc are available in the source place.
 */
const checkTransitionEnabled = ({ transitionId, arcs, placeMarkingsById }: { transitionId: string; arcs: ArcType[]; placeMarkingsById: PlaceMarkingsById }): boolean => {
  ...
  return incomingArcs.every((arc) => {
    ...
    return Object.entries(arc.data?.tokenWeights ?? {}).every(([tokenTypeId, requiredWeight]) => {
...
export const SimulationContextProvider = ({ children }: SimulationProviderProps) => {
  ...
  const [simulationLogs, setSimulationLogs] = useState<Array<{ id: string; text: string }>>([]);
  ...
  useEffect(() => {
    resetSimulation();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- resetSimulation changes when nodes change, so we only need nodes
  }, [petriNetDefinition.nodes]);
  ...
  const fireNextStep = useCallback(() => {
    ...
    const updatedMarkings: PlaceMarkingsById = JSON.parse(JSON.stringify(placeMarkingsById));

    for (const transition of enabledTransitions) {
      const transitionNode = petriNetDefinition.nodes.find((node): node is TransitionNodeType => node.id === transition.id && node.data.type === "transition");
      ...
      if (transitionNode.data.conditions?.length) {
        ...
        let selectedCondition: TransitionCondition | null = null;

        for (const condition of transitionNode.data.conditions) {
          cumulativeProbability += typeof condition.probability === "number" ? condition.probability : 0;
        ...
        outgoingArc = petriNetDefinition.arcs.find((arc) => arc.source === transition.id && arc.id === selectedCondition.outputEdgeId);

        addLogEntry(`Transition "${transitionNode.data.label}" resulted in condition: ${selectedCondition.name} (${selectedCondition.probability}% probability)`);
      } else {
        /**
         * If the transition has only one output, we can just use that.
         */
        outgoingArc = petriNetDefinition.arcs.find((arc) => arc.source === transition.id);
      }

      if (!outgoingArc) {
        throw new Error(`Outgoing edge for transition ${transition.id} not found`);
      }

      const incomingArcs = petriNetDefinition.arcs.filter((arc) => arc.target === transition.id);

      if (incomingArcs.length === 0) {
        throw new Error(`Expected at least 1 incoming arc for transition ${transition.id}`);
      }
      ...
      for (const incomingArc of incomingArcs) {
        for (const [tokenTypeId, weight] of Object.entries(incomingArc.data?.tokenWeights ?? {})) {
          if (!weight) {
            continue;
          }
          ...
          if (!sourceMarkings) {
            throw new Error(`Could not find token counts for place ${incomingArc.source}`);
          }

          sourceMarkings[tokenTypeId] = (sourceMarkings[tokenTypeId] ?? 0) - weight;
      ...
      for (const [tokenTypeId, weight] of Object.entries(outgoingArc.data?.tokenWeights ?? {})) {
        if (!weight) {
          continue;
        }
        ...
        if (!targetMarkings) {
          throw new Error(`Could not find token counts for place ${outgoingArc.target}`);
        }

        targetMarkings[tokenTypeId] = (targetMarkings[tokenTypeId] ?? 0) + weight;
  ...
  }, [petriNetDefinition.arcs, petriNetDefinition.nodes, addLogEntry, simulationSpeed, placeMarkingsById]);
  ...
  const contextValue = useMemo(
    ...
    [currentStep, fireNextStep, isSimulating, placeMarkingsById, resetSimulation, simulationLogs, simulationSpeed]
  );
