...
import ReactFlow, { Background, ConnectionLineType, ReactFlowProvider, useReactFlow } from "reactflow";

import { Arc } from "./petrinaut/arc";
import { ArcEditor } from "./petrinaut/arc-editor";
import { createArc } from "./petrinaut/create-arc";
import { EditorContextProvider, type MutatePetriNetDefinition, useEditorContext } from "./petrinaut/editor-context";
...
import type { ArcData, ArcType, MinimalNetMetadata, NodeData, NodeType, ParentNet, PetriNetDefinitionObject, PlaceNodeData, PlaceNodeType, TokenCounts, TokenType, TransitionCondition, TransitionNodeData, TransitionNodeType } from "./petrinaut/types";
...
export type { ArcData, ArcType, MinimalNetMetadata, NodeData, NodeType, ParentNet, PetriNetDefinitionObject, PlaceNodeData, PlaceNodeType, TokenCounts, TokenType, TransitionCondition, TransitionNodeData, TransitionNodeType };
...
type DraggingStateByNodeId = Record<string, { dragging: boolean; position: { x: number; y: number } }>;

const PetrinautInner = ({ hideNetManagementControls }: { hideNetManagementControls: boolean }) => {
  const canvasContainer = useRef<HTMLDivElement>(null);

  const [reactFlowInstance, setReactFlowInstance] = useState<ReactFlowInstance<NodeData, ArcData> | null>(null);

  const { createNewNet, petriNetDefinition, mutatePetriNetDefinition, title } = useEditorContext();
  ...
  const [selectedArc, setSelectedArc] = useState<(ArcType & { position: { x: number; y: number } }) | null>(null);
   ...
   */
  const [draggingStateByNodeId, setDraggingStateByNodeId] = useState<DraggingStateByNodeId>({});
  ...
  const isValidConnection = useCallback(
    (connection: Connection) => {
      const sourceNode = petriNetDefinition.nodes.find((node) => node.id === connection.source);
      const targetNode = petriNetDefinition.nodes.find((node) => node.id === connection.target);
  ...
  const onConnect = useCallback(
    ...
    [petriNetDefinition.arcs, isValidConnection, petriNetDefinition.tokenTypes, mutatePetriNetDefinition]
  );
  ...
  const onDrop = useCallback(
    (event: DragEvent<HTMLDivElement>) => {
      ...
      const nodeType = event.dataTransfer.getData("application/reactflow") as "place" | "transition";
  ...
  const handleUpdateEdgeWeight = useCallback(
    (edgeId: string, tokenWeights: { [tokenTypeId: string]: number | undefined }) => {
      mutatePetriNetDefinition((existingNet) => {
  ...
  const handleUpdateTransition = useCallback(
    (transitionId: string, transitionData: Omit<TransitionNodeData, "type">) => {
      mutatePetriNetDefinition((existingNet) => {
        const transitionNode = existingNet.nodes.find((node): node is TransitionNodeType => node.id === transitionId && node.type === "transition");
  ...
  const selectedPlace = useMemo(() => {
    ...
    const place = petriNetDefinition.nodes.find((node): node is PlaceNodeType => node.id === selectedPlaceId && node.data.type === "place");
  ...
  const nodesForReactFlow = useMemo(() => {
    return petriNetDefinition.nodes.map((node) => {
      ...
      return {
        ...
        position: draggingState?.dragging ? draggingState.position : node.position,
      };
    });
    ...
    <Stack sx={{ height: "100%" }}>
      ...
      <Stack direction="row" sx={{ height: "100%", userSelect: "none" }}>
        ...
        >
          ...
          {selectedTransition && (
            <TransitionEditor
              ...
              outgoingEdges={petriNetDefinition.arcs
                ...
                .map((edge) => {
                  const targetNode = petriNetDefinition.nodes.find((node) => node.id === edge.target);
                  return {
                    id: edge.id,
                    source: edge.source,
                    target: edge.target,
          ...
          {selectedPlace && <PlaceEditor position={selectedPlacePosition ?? { x: 0, y: 0 }} selectedPlace={selectedPlace} tokenTypes={petriNetDefinition.tokenTypes} onClose={() => setSelectedPlaceId(null)} onUpdateInitialTokens={handleUpdateInitialTokens} onUpdateNodeLabel={handleUpdateNodeLabel} />}

          {selectedArc && <ArcEditor arcId={selectedArc.id} tokenWeights={selectedArc.data?.tokenWeights ?? {}} position={selectedArc.position} onUpdateWeights={handleUpdateEdgeWeight} />}

          <ReactFlow nodes={nodesForReactFlow} edges={petriNetDefinition.arcs} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onInit={onInit} onDrop={onDrop} onDragOver={onDragOver} onNodeClick={onNodeClick} onEdgeClick={onEdgeClick} onPaneClick={handlePaneClick} defaultViewport={{ x: 0, y: 0, zoom: 1 }} snapToGrid snapGrid={[15, 15]} connectionLineType={ConnectionLineType.SmoothStep} proOptions={{ hideAttribution: true }}>
          ...
          {!hideNetManagementControls && (
            <Stack direction="row" spacing={1} sx={{ position: "absolute", bottom: 16, right: 16, zIndex: 100 }}>
              <Button onClick={handleLoadExample} size="xs" variant="tertiary">
                Load Example
              </Button>
              <input type="file" ref={fileInputRef} onChange={handleLoadFromPnml} accept=".pnml,.xml" style={{ display: "none" }} />
...
export type PetrinautProps = {
  /**
   * The Automerge document handle, used for creating refs for annotation lookup.
   */
